...
pib_sectores_outsample <- pib_sectores |>
  filter(fecha > "2019-09-01", year < 2022)
...
by_sectores <- pib_sectores_sample %>%
  ...
  select(fecha, year, trimestre, sector, pib_2007, crecimiento, recesion) %>% 
  group_by(sector) %>% 
  nest()
...
by_sectores <- by_sectores %>% 
  mutate(
    ...
    model = map(
      ts,
      \(ts) auto.arima(ts[, "crecimiento"], xreg = ts[, "recesion"], seasonal = TRUE)
    ),
    ...
    fcast = map(
      ...
      forecast::forecast,
      h = 8,
      xreg = pib_sectores_outsample$recesion[1:8]
    ),
    ...
    # rmse historico --- --- ---
    errores_rmse = map(
      ts, \(ts) error_historico(ts[, "crecimiento"], n.valid = 8)
    ),
    
    # error absoluto --- --- ---
    errores_me = map(
      ts, \(ts) error_historico(ts[, "crecimiento"], n.valid = 8, medida_error = "ME")
    )
  )
...
fanplots_sectores <- pmap(
  list(
    serie = by_sectores[["ts"]] %>% map(~tail(.x, 52)) |> map(~ .x[, "crecimiento"]),
